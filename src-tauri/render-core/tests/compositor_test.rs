...
fn padding_insets_video_over_background() {
    ...
    let video = RgbaFrame {
        width: 64,
        height: 48,
        data: vec![255, 0, 0, 255].repeat(64 * 48),
    };
    let out = c
        .render_frame(&gpu, &framing, &video, None, 200, 120)
        .unwrap();
    let p = |x: u32, y: u32| {
    ...
    assert_eq!(
        p(10, 60)[2],
        255,
        "left padding should be blue bg, got {:?}",
        p(10, 60)
    );
...
fn rounded_corners_show_background_at_corner() {
    ...
    let video = RgbaFrame {
        width: 200,
        height: 120,
        data: vec![255, 0, 0, 255].repeat(200 * 120),
    };
    let out = c
        .render_frame(&gpu, &framing, &video, None, 200, 120)
        .unwrap();
    ...
    assert_eq!(
        p(1, 1)[2],
        255,
        "corner should be background (blue) due to radius, got {:?}",
        p(1, 1)
    );
    let center = p(100, 60);
    assert!(
...
fn drop_shadow_darkens_below_video() {
    ...
    let video = RgbaFrame {
        width: 100,
        height: 60,
        data: vec![255, 0, 0, 255].repeat(100 * 60),
    };
    let with = c
        .render_frame(&gpu, &mk(0.5), &video, None, 200, 160)
        .unwrap();
    let without = c
        .render_frame(&gpu, &mk(0.0), &video, None, 200, 160)
        .unwrap();
